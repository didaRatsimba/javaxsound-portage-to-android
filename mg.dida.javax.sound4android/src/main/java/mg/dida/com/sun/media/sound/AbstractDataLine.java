...
abstract class AbstractDataLine extends AbstractLine implements DataLine {
	...
	public void open(AudioFormat format, int bufferSize) throws LineUnavailableException {
		...
		synchronized (mixer) {
			if (Printer.trace) {
				Printer.trace("> AbstractDataLine.open(format, bufferSize) (class: "+getClass().getName());
			}
			...
			if (!isOpen()) {
				...
				if (Printer.debug) {
					Printer.debug("  need to open the mixer...");
				}
				...
				if (Printer.debug) {
					Printer.debug("  dataline already open");
				}
	...
	public void start() {
		...
		synchronized(mixer) {
			if (Printer.trace) {
				Printer.trace("> "+getClass().getName()+".start() - AbstractDataLine");
			}
	...
	public void stop() {
		...
		synchronized(mixer) {
			if (Printer.trace) {
				Printer.trace("> "+getClass().getName()+".stop() - AbstractDataLine");
			}
	...
	protected void setEOM() {
		...
		setStarted(false);
		if (Printer.trace) {
			Printer.trace("< AbstractDataLine: setEOM() completed");
		}
	}
	...
	public void open() throws LineUnavailableException {
		...
		if (Printer.trace) {
			Printer.trace("> "+getClass().getName()+".open() - AbstractDataLine");
		}
	...
	public void close() {
		...
		synchronized (mixer) {
			if (Printer.trace) {
				Printer.trace("> "+getClass().getName()+".close() - in AbstractDataLine.");
			}
