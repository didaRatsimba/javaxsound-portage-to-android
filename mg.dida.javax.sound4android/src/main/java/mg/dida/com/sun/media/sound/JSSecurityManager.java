...
import mg.dida.com.sun.media.sound.misc.Service;
import mg.dida.javax.sound.share.classes.javax.sound.sampled.AudioPermission;
...
class JSSecurityManager {
	...
	static void checkRecordPermission() throws SecurityException {
		if(Printer.trace) {
			Printer.trace("JSSecurityManager.checkRecordPermission()");
		}
	...
	static void loadLibrary(final String libName) {
		try {
			if (hasSecurityManager()) {
				if(Printer.debug) {
					Printer.debug("using security manager to load library");
				}
				...
				if(Printer.debug) {
					Printer.debug("not using security manager to load library");
				}
				System.loadLibrary(libName);
			}
			if (Printer.debug) {
				Printer.debug("loaded library " + libName);
			}
		} catch (UnsatisfiedLinkError e2) {
			if (Printer.err) {
				Printer.err("UnsatisfiedLinkError loading native library " + libName);
			}
			throw(e2);
		}
	}
	...
	static String getProperty(final String propertyName) {
		...
		if (hasSecurityManager()) {
			if(Printer.debug) {
				Printer.debug("using JDK 1.2 security to get property");
			}
			try{
				PrivilegedAction action = new PrivilegedAction() {
					@Override
				...
				if(Printer.debug) {
					Printer.debug("not using JDK 1.2 security to get properties");
				}
	...
	static void loadProperties(final Properties properties,
		...
		if(hasSecurityManager()) {
			try {
				...
				AccessController.doPrivileged(action);
				if(Printer.debug) {
					Printer.debug("Loaded properties with JDK 1.2 security");
				}
			} catch (Exception e) {
				if(Printer.debug) {
	...
	private static void loadPropertiesImpl(Properties properties,
			String filename) {
		if(Printer.trace) {
			Printer.trace(">> JSSecurityManager: loadPropertiesImpl()");
		}
		...
		if(Printer.trace) {
			Printer.trace("<< JSSecurityManager: loadPropertiesImpl() completed");
		}
	}
	...
	private static ThreadGroup getTopmostThreadGroupImpl() {
		if(Printer.trace) {
			Printer.trace(">> JSSecurityManager: getTopmostThreadGroupImpl()");
		}
	...
	static Thread createThread(final Runnable runnable,
		...
		if(hasSecurityManager()) {
			...
			if(Printer.debug) {
				Printer.debug("created thread with JDK 1.2 security");
			}
		} else {
			if(Printer.debug) {
				Printer.debug("not using JDK 1.2 security");
			}
